...
def load_pil_image(filename):
    ...
    img = Image.open(get_image_path(filename))
    img.load()
    return img
...
class TestPilInterface(unittest.TestCase):
    ...
    def test_pil_image_created_from_imageutils_is_the_same_as_loaded_image(self):
        for filename in get_image_paths('uncompressed'):
            try:
                ...
                continue
            bmp2 = imageutils.to_pil(load_image(filename))
    ...
    def test_pil_image_created_from_compressed_is_the_same_as_loaded_image(self):
        for filename in get_image_paths('compressed'):
            bmp1 = load_image(filename)
    ...
    def test_load_from_path_or_pil_with_path_returns_image(self):
        ...
        img = imageutils.load_image_from_path_or_pil(get_image_path(path))
